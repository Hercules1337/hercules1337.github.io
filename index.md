# Hercules — Offensive Security Notes & OSCP Journey

Welcome to my security lab notebook.  
This site is a hybrid: a clean overview for recruiters and a deep technical workspace where I document everything I learn while preparing for the OSCP.

I use this space to track my progress across Hack The Box, PortSwigger Academy, and other hands‑on labs.  
If you're a recruiter or hiring manager, the sections below highlight my focus areas and showcase my methodology.

---

## 🔹 About Me
I'm an aspiring penetration tester with a strong interest in offensive security, vulnerability analysis, and hands‑on lab environments.  
My learning approach is simple: **enumerate deeply, document everything, and continuously refine my methodology.**

I'm currently working through:
- Hack The Box (Tier 0 → Tier 2)
- PortSwigger Web Security Academy
- OSCP‑style lab environments
- Privilege escalation practice (Linux & Windows)

---

## 🔹 Skills & Focus Areas
- Network enumeration (Nmap, SMB, FTP, SSH, WinRM)
- Web exploitation (LFI/RFI, SQLi, CSRF, auth bypass)
- Linux & Windows privilege escalation
- Scripting & automation (Bash, Python)
- Report writing & documentation
- Real‑world methodology and attack chains

---

## 🔹 Showcase Writeups (Polished)
A small selection of clean, recruiter‑friendly writeups will go here.

Coming soon…

👉 **[Showcase](/showcase/)**

---

## 🔹 Full Notes (Raw & In‑Progress)
These are my detailed notes — commands, dead ends, screenshots, and full methodology.  
This is where I do the real work.

👉 **[View all notes](notes/)**

---

## 🔹 Contact
If you'd like to connect or discuss opportunities, feel free to reach out via GitHub.






